...
const completedMissions = computed(
  () => props.missions?.filter((m) => m.finished).length || 0
)
const inProgressMissions = computed(
  () => props.missions?.filter((m) => m.has_attempt && !m.finished).length || 0
)
const notStartedMissions = computed(
  () => props.missions?.filter((m) => !m.has_attempt).length || 0
)
</script>
      ...
      <div class="wrap">
        <div class="stats-grid">
          ...
          <div class="stat-card">
            ...
            <div class="stat-content">
              <div class="stat-value">{{ inProgressMissions }}</div>
              <div class="stat-label">Sedang Berjalan</div>
            </div>
          </div>

          <div class="stat-card">
            <div class="stat-icon" style="background: rgba(107,114,155,.15); color: #6b727b">
              <Zap :size="20" :stroke-width="2" />
            </div>
            <div class="stat-content">
              <div class="stat-value">{{ notStartedMissions }}</div>
      ...
      <div class="wrap">
        <div v-if="totalMissions > 0" class="missions-grid">
          <article
            ...
            :style="{ '--delay': i * 50 + 'ms' }"
          >
            <div class="mission-badge" :style="{ color: getMissionStatusColor(mission), background: getMissionStatusBg(mission) }">
              <component :is="getStatusIcon(mission)" :size="16" :stroke-width="2" />
...
.mission-card {
  ...
  transition: opacity 0.4s var(--delay, 0ms) ease,
